...
const Index = () => {
  ...
  return (
    <main className="relative h-screen min-h-[100svh] overflow-hidden bg-background text-foreground">
      <div className="absolute inset-0 z-0">
        <KrishiMap selectedId={selectedId} language={language} onSelect={setSelectedId} />
      </div>

      <div className="pointer-events-none absolute inset-x-0 top-0 z-[850] h-28 bg-gradient-to-b from-background/70 to-background/0" />

      <header className="pointer-events-none fixed left-0 right-0 top-0 z-[950] flex items-start justify-between gap-3 p-4 md:p-6">
        <section className="pointer-events-auto max-w-[calc(100vw-1.5rem)] rounded-lg border border-glass-border bg-glass/90 px-4 py-3 text-glass-foreground shadow-glass backdrop-blur-panel md:px-5">
        ...
        <nav className="pointer-events-auto flex shrink-0 items-center gap-2 rounded-lg border border-glass-border bg-glass/90 p-2 text-glass-foreground shadow-glass backdrop-blur-panel">
      ...
      <div className="fixed bottom-[60vh] left-4 z-[850] hidden rounded-lg border border-glass-border bg-glass/90 p-3 text-glass-foreground shadow-glass backdrop-blur-panel md:bottom-6 md:block">
